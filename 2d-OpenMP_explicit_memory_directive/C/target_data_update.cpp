...
double some_computation(double x){
  return(2.0*x);
}
...
double final_computation(double x, double y){
  return(x+y);
}
...
#pragma omp target
#pragma omp teams parallel for reduction(+:res)
    for (int i=0; i<N; i++)
      res += final_computation(input[i], tmp[i]);
  }
